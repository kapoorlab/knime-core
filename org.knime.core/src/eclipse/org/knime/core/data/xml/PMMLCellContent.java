...
public class PMMLCellContent extends XMLCellContent implements PMMLValue {
    ...
    public String getPMMLVersion() {
        try (AutoclosableSupplier<Document> supplier = getDocumentSupplier()) {
            Node pmml = supplier.getObject().getElementsByTagName(PMMLPortObject.PMML_ELEMENT).item(0);
    ...
    public List<Node> getModels(final PMMLModelType type) {
        ...
        try (AutoclosableSupplier<Document> supplier = getDocumentSupplier()) {
            Node pmml = supplier.getObject().getElementsByTagName("PMML").item(0);
